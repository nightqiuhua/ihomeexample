...
def login():
    ...
    try:
        ...
        access_num = bytes.decode(access_num)
    except Exception as e:
        current_app.logger.error(e)
    else:
        if access_num is not None and int(access_num) >= constants.LOGIN_ERROR_MAX_TIEMS:
    ...
    if user is None or not user.check_password(password):
        # 如果验证失败，记录错误次数，返回信息
        try:
            redis_store.incrby("access_num_{}".format(user_ip))
